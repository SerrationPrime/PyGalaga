import pygame
from multiplayer_menu_btnfunc import action_connectgame, action_hostgame, action_tournament
...
def button_tournamnet():
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()
    if 140 + 295 > mouse[0] > 140 and 515 + 50 > mouse[1] > 515:
        s = pygame.Surface((295, 50))
        s.set_alpha(50)
        gameDisplay.blit(s, (140, 515))
        if click[0] == 1:
            action_tournament()
            pygame.quit()
            quit()
...
crashed = False
img = pygame.image.load('newmultiplayer_menu.png')
